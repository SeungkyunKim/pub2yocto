...
```shell
dart run pub2yocto -i path/to/pubspec.lock -o path/to/output.inc
```

### Options:

- `-i, --input`: Specify the input file. Defaults to `pubspec.lock`.
- `-o, --output`: Specify the output file name for the generated recipe. If not provided, `pub2yocto` generates a `.inc` file named after the project defined in `pubspec.yaml`.
...
### Translation Example: From `pubspec.yaml` to `.inc`
#### pubspec.yaml
```yaml
dependencies: 
  args: ^2.7.0
